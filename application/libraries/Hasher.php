...
class Hasher {
    ...
    */
    public function okLogin ($username, $password)
    {
    	$mythis = & get_instance();
  		$mythis->load->model('loginm');
   		$user = $mythis->loginm->validateCredentials($username);

        $verify = $this->validatePassword($password, @$user->password );

        if ($verify)
        {
        	$mythis->session->set_userdata(
        								array(
					                    'id' => $user->userID,
